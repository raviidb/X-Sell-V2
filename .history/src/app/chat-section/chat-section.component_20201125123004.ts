...
export class ChatSectionComponent implements OnInit {
  ...
  requiredMsg: string;
  value1 = 150000; value2 = 35000; value3 = 35000;
  maxValue: any;
  userDetails: any = [];
  empType = [
  ...
  showMsg100: boolean; showMsg101: boolean; showMsg102: boolean; showMsg103: boolean; showMsg104: boolean; showMsg105: boolean;showMsg106: boolean;
  ...
  ngOnInit() {
    ...
    setTimeout(() => { this.showMsg4 = true; }, 3400);
  ...
  onContinue(event) {
    ...
    this.requiredMsg = '';
    let static_name = 'DMI Customer'
    if (event == 2) {
      this.service.getStep1Info(this.mobileNumber, static_name).subscribe(res => {
        this.routerKey = res;
        this.otpOption = true;
  ...
  onSelectLoanAmount() {
    ...
    this.service.getStep2Info(this.routerKey, '50000', loanDetails).subscribe(res => {
    });
  }
  ...
  onPanSubmit() {
    ...
    this.onSelectLoanAmount();
    this.onSubmitEmail();
  }

  onSubmitEmail() {
    this.service.getStep7Info(this.routerKey, 'Salaried', '35000', this.userPan, 'dmilos@dmifinance.in').subscribe(res => {});
  }
  ...
  onSubmitEmpInfo() {
    ...
    //   return;
    // }
    this.service.getStep4Info(this.routerKey, '24', 'DMI Finance',).subscribe(res => {
      this.service.getStep5Info(this.routerKey, 'Office Address', '-',
        'Express Building', '3rd floor', 'Near ITO Metro', '110001', 'Delhi', 'India', 'Delhi', '15').subscribe(res => {
  ...
  finalForm() {
    ...
    setTimeout(() => { this.showMsg106 = true; }, 1300);
    setTimeout(() => { this.showMsg44 = true; }, 2100);
    setTimeout(() => { this.showMsg45 = true; }, 2900);
    setTimeout(() => { this.showMsg38 = true; }, 3700);
    setTimeout(() => { this.showMsg39 = true; }, 4500);
  }
  ...
  kycModal(event) {
    ...
    if (event == 1) {
      let kycDetails = {
        ...
        "Borrower_Email": 'dmilos@dmifinance.in',
        "Loan_Number": "NULL",
        "Contact_ID": "NULL",
      ...
      this.service.initiateEMandate(this.routerKey,
        this.userDetails.UserInfo[0].Google_Application_Id == null ? "null" : this.userDetails.UserInfo[0].Google_Application_Id).subscribe(res => {
          let url = 'https://dmi.vistaconnect.com/e-mandate-v1.1/?key=5fbdff583089b';
        ...
        this.service.getLDSUrl(ldsDetails).subscribe(res => {
          ...
          modalref.componentInstance.sendStatus.subscribe(res => {
            if (res.status == 'Success') {
                ...
                setTimeout(() => { this.showMsg59 = true; 
                }, 2100, this.service.submitLoanApplication(this.routerKey, this.userDetails.UserInfo[0].Google_Application_Id).subscribe(res => {
                  this.loanNumber = res.data.partnerApplicationReferenceId;
                  this.onLoanCReationJourney();
                }))
                // if(this.showMsg59 == true){
